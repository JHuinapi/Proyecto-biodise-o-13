...
const bool ACTIVE_HIGH = true;   // Si tu receptor entrega LOW al pulsar, pon false y usa INPUT_PULLUP
...
const byte SPEED_PWM = 180;          // 0..255
const unsigned long RUN_MS = 2000;   // 2 segundos
...
void setup() {
  ...
  // RF botones
  if (ACTIVE_HIGH) {
    pinMode(BTN_FWD, INPUT);  // si hay ruido, añade pull-down externo 10k a GND
    pinMode(BTN_REV, INPUT);
  } else {
    pinMode(BTN_FWD, INPUT_PULLUP); // activo en LOW
...
void loop() {
  ...
  if (now - tIMU >= IMU_PERIOD_MS) {
    ...
    if (mpu.update()) {
      ...
      // (convenciones típicas; ajusta según tu montaje)
      ...
      // Si no hay datos nuevos, no imprimas para no saturar
    }
  }
}